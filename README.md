...
[Разные сценарии которые могуг произойти с кластером](</docs/Сценарии использования.md>)
...
```bash
    ...
    docker-compose up -d
 ```
## 3. Настройка переменных Ansible

Переменные окружения все находятся в `infrastructure/ansible/group_vars/`.

есть `vault.yml.example` в котором описано какие секреты должны быть помещены в `infrastructure/ansible/group_vars/vault.yml`

### 3.1. Шифрование секретов
...
### 3.2. Основные переменные и настройки подключения

Базовая конфигурация разделена на два уровня:
**1. Настройки среды в инвентаре (`infrastructure/ansible/inventory.ini`)**
В блоке `[all:vars]` задаются параметры доступа и архитектура хоста. Проверьте их перед запуском:
* `target_arch`: Укажите `arm64` или `amd64`

**2. Глобальные переменные кластера (`infrastructure/ansible/group_vars/all/vars.yml`)**
Здесь задаются сетевые настройки отказоустойчивости:
* `storage_vip`, `db_vip`, `gitea_vip`: Виртуальные IP-адреса (Floating IP), которыми управляет Keepalived для соответствующих слоев системы.

## 4. Развертывание кластера

Запуск полной цепочки установки всех ролей: (Желательно включить впнчик с иностранным сервером)
