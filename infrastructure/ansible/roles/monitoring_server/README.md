...
2. Замену плейсхолдеров `${DS_PROMETHEUS}` и `${DS_LOKI}` (регистронезависимо) на соответствующие UID, заданные в переменных. Это гарантирует, что графики заработают сразу после установки без ручной настройки.
